...
const Categories = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/admin/categories/${editingId}`, { name: newName });
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error(err);
      alert('Failed to delete. Ensure no products or attributes are linked to this category first.');
    }
  };
  ...
  return (
    <div>
      <div className="admin-header-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '1.8rem', margin: 0 }}>Categories</h1>
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #eaeaea', overflow: 'hidden' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '600px' }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={4} style={{ padding: '32px', textAlign: 'center' }}>Loading categories...</td></tr>
            ) : categories.length === 0 ? (
              <tr><td colSpan={4} style={{ padding: '32px', textAlign: 'center', color: '#888' }}>No categories found.</td></tr>
            ) : categories.map(cat => (
              <tr key={cat.id} style={{ borderBottom: '1px solid #eaeaea' }}>
                <td style={{ padding: '12px 16px', fontWeight: 500 }}>{cat.name}</td>
                <td style={{ padding: '12px 16px', color: '#666', fontFamily: 'monospace' }}>{cat.slug}</td>
                <td style={{ padding: '12px 16px' }}>{cat._count?.products || 0}</td>
                <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                  <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                    <button onClick={() => handleEdit(cat)} style={{ padding: '6px', background: 'none', border: '1px solid #ccc', borderRadius: '4px', cursor: 'pointer', color: '#555' }}>
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(cat.id)} style={{ padding: '6px', background: '#fee2e2', border: '1px solid #f87171', borderRadius: '4px', cursor: 'pointer', color: '#dc2626' }}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', padding: '32px', borderRadius: '12px', width: '100%', maxWidth: '400px' }}>
            <h3 style={{ margin: '0 0 16px' }}>{editingId ? 'Edit Category' : 'Add Category'}</h3>
